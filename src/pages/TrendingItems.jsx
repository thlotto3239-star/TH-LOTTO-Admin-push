...
export default function TrendingItems() {
  ...
  const load = async () => {
    const { data } = await supabase.from('trending_items').select('*').order('display_order')
    setRows(data || [])
    setLoading(false)
  }
  ...
  const save = async () => {
    setWorking(true)
    const { id, updated_at, created_at, ...fields } = modal
    let error
    if (id) {
      ({ error } = await supabase.from('trending_items').update(fields).eq('id', id))
    } else {
      ({ error } = await supabase.from('trending_items').insert(fields))
    }
    setWorking(false)
  ...
  const moveOrder = async (id, dir) => {
    ...
    await supabase.from('trending_items').update({ display_order: b.display_order }).eq('id', a.id)
    await supabase.from('trending_items').update({ display_order: a.display_order }).eq('id', b.id)
    load()
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-on-surface">รายการมาแรง</h1>
          <p className="text-on-surface-variant text-sm">จัดการข้อมูลแนะนำในหมวด "มาแรง" หน้าแรกของผู้เล่น</p>
        </div>
        <button onClick={() => setModal({ ...EMPTY, display_order: rows.length + 1 })}
          ...
          <Plus size={15} /> เพิ่มรายการมาแรง
        </button>
      </div>

      <div className="space-y-3">
        {rows.length === 0 && (
          <div className="text-center py-12 text-outline glass-panel rounded-2xl">
            ยังไม่มีรายการมาแรง (ระบบจะแสดงหวย 1 นาทีเป็นค่าเริ่มต้นที่หน้าเว็บ)
          </div>
        )}
        {rows.map((r, i) => (
          <div key={r.id} className={`glass-panel rounded-2xl shadow-glass p-4 flex items-center gap-4 ${!r.is_active ? 'opacity-50' : ''}`}>
            {r.image_url ? (
              <img src={r.image_url} alt={r.title} className="w-16 h-16 object-cover rounded-xl border border-outline-variant flex-shrink-0" />
            ) : (
              <div className="w-16 h-16 rounded-xl bg-surface-container flex items-center justify-center text-outline">
                <span className="material-symbols-outlined">image</span>
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-on-surface">{r.title || 'ไม่มีชื่อ'}</div>
              {r.code && <div className="text-xs text-on-surface-variant">{r.code}</div>}
              {r.link && <div className="text-xs text-secondary truncate mt-1">{r.link}</div>}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="glass-panel rounded-2xl shadow-capsule w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-on-surface">{modal.id ? 'แก้ไข' : 'เพิ่ม'}รายการมาแรง</h3>
              <button onClick={() => setModal(null)}><X size={20} className="text-outline" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-on-surface-variant mb-1 block">ชื่อหวย (Title)</label>
                <input type="text" value={modal.title || ''} placeholder="เช่น หวยไทย 1 นาที"
              ...
              <div>
                <label className="text-xs font-medium text-on-surface-variant mb-1 block">คำอธิบายย่อย (Code/Subtitle)</label>
                <input type="text" value={modal.code || ''} placeholder="เช่น ออกผลทุก 1 นาที ตลอด 24 ชั่วโมง"
                  onChange={e => setModal(m => ({ ...m, code: e.target.value }))}
                  className="w-full bg-surface-container-low border-none rounded-full px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="text-xs font-medium text-on-surface-variant mb-1 block">ลิงก์ปลายทาง (Link)</label>
                <input type="text" value={modal.link || ''} placeholder="เช่น /lottery-list"
                  onChange={e => setModal(m => ({ ...m, link: e.target.value }))}
                  className="w-full bg-surface-container-low border-none rounded-full px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="text-xs font-medium text-on-surface-variant mb-1 block">ลำดับการแสดงผล</label>
                <input type="number" value={modal.display_order ?? 0}
                  onChange={e => setModal(m => ({ ...m, display_order: Number(e.target.value) }))}
                  className="w-full bg-surface-container-low border-none rounded-full px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>

              {/* File Upload */}
              <div>
                <label className="text-xs font-medium text-on-surface-variant mb-1 block">รูปภาพ/ไอคอน (แนะนำ 1:1)</label>
                <label className="flex items-center justify-center gap-2 border-2 border-dashed border-outline-variant rounded-xl py-4 cursor-pointer hover:border-primary hover:bg-primary/5 transition-all">
                  ...
                  {uploading ? (
                    ...
                    <><Upload size={18} className="text-outline" /> <span className="text-sm text-on-surface-variant">เลือกไฟล์ภาพ (สูงสุด 5MB)</span></>
                  )}
                </label>
              ...
              {modal.image_url && (
                <div>
                  ...
                  <img src={modal.image_url} alt="" className="w-24 h-24 rounded-xl object-cover border border-outline-variant" />
                </div>
              )}
